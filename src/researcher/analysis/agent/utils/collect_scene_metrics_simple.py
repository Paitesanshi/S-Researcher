...
class SimpleSceneMetricsCollector:
    def __init__(
        ...
        output_dir: str,
        latest_runs: int = 1,
        group_types: Optional[Dict[str, Any]] = None,
    ):
        ...
        # group_types can be:
        # - Rich structure: group_name -> {"type": str, "replicate_of": str, "all_runs": list, ...}
        # - Simple structure (legacy): group_name -> type_string
        self.group_types = group_types or {}
    ...
    def _discover_group_directories(self) -> List[Path]:
        """
        Discover group directories to process.
        If group_types is provided, only process groups defined in group_types.
        Otherwise, scan all directories under groups_base_path.
        """
        group_dirs: List[Path] = []
        if not self.groups_base_path.exists():
            return group_dirs

        # If group_types is provided, only process groups defined there
        if self.group_types:
            for group_name in self.group_types.keys():
                group_path = self.groups_base_path / group_name
                if group_path.is_dir():
                    group_dirs.append(group_path)
                else:
                    logger.warning(f"Group directory not found for defined group: {group_name}")
        else:
    ...
    def _get_group_info(self, group_name: str) -> Dict[str, Any]:
        """
        Extract group info from group_types, handling both rich and simple structures.
        Returns: {
            "group_type": str,
            "replicate_of": str or None,
            "treatment_id": str (the base group name for aggregation),
        }
        """
        info = self.group_types.get(group_name)
        if info is None:
            return {
                "group_type": "unknown",
                "replicate_of": None,
                "treatment_id": group_name,
            }

        # Handle rich structure: {"type": str, "replicate_of": str, ...}
        if isinstance(info, dict):
            group_type = info.get("type", "unknown")
            replicate_of = info.get("replicate_of")
            # treatment_id is the base group for aggregation
            # For replicates, it's replicate_of; for base groups, it's the group itself
            treatment_id = replicate_of if replicate_of else group_name
            return {
        ...
        # Handle simple structure (legacy): group_type is a string
        group_type = str(info)
        # Try to infer replicate_of from naming pattern for legacy data
        replicate_of = None
        treatment_id = group_name
        if group_type == "replicate" and "_rep_" in group_name:
            # Parse "treatment_group_001_rep_1" -> "treatment_group_001"
            parts = group_name.rsplit("_rep_", 1)
            if len(parts) == 2:
                replicate_of = parts[0]
                treatment_id = replicate_of
    ...
    def _collect_step_metrics_once(self, group_name: str, run_name: str, step_name: str, step_dir: Path) -> Dict[str, Any]:
        ...
        # Get rich group info once for this group
        ...
        for category_dir in scene_metrics_dir.iterdir():
            ...
            try:
                ...
                item = {
                    ...
                    "replicate_of": group_info["replicate_of"],
                    "treatment_id": group_info["treatment_id"],
                    "run": run_name,
                    "step": step_name,
    ...
    def _build_treatment_summary(self, category_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        Build a summary of treatments and their replicates for easier downstream aggregation.
        Returns: {
            "treatments": {
                "treatment_group_001": {
                    "type": "treatment",
                    "runs": ["treatment_group_001", "treatment_group_001_rep_1", "treatment_group_001_rep_2"],
                    "data_points": 9,
                }
            },
            "replication_info": {
                "has_replicates": true,
                "total_treatments": 6,
                "runs_per_treatment": 3,
            }
        }
        """
        treatments: Dict[str, Dict[str, Any]] = {}
        for item in category_data:
            treatment_id = item.get("treatment_id", item["group_name"])
            if treatment_id not in treatments:
                # Get the type from the base group
                base_info = self._get_group_info(treatment_id)
                treatments[treatment_id] = {
                    "type": base_info["group_type"],
                    "runs": set(),
                    "data_points": 0,
                }
            treatments[treatment_id]["runs"].add(item["group_name"])
            treatments[treatment_id]["data_points"] += 1

        # Convert sets to sorted lists
        for tid in treatments:
            treatments[tid]["runs"] = sorted(list(treatments[tid]["runs"]))

        # Compute replication info
        runs_counts = [len(t["runs"]) for t in treatments.values()]
        has_replicates = any(c > 1 for c in runs_counts)
        ...
        return {
            ...
            "replication_info": {
                ...
                "runs_per_treatment": max(runs_counts) if runs_counts else 0,
            }
        }
    ...
    def save(self) -> None:
        ...
        for category, category_data in collected_data.items():
            ...
            # Build treatment-level summary for aggregation
            treatment_summary = self._build_treatment_summary(category_data)
            ...
            output_data = {
                **self._generate_file_header(category),
                ...
                "summary": {
                    ...
                    "treatments_with_data": sorted(list({item.get('treatment_id', item['group_name']) for item in category_data})),
            ...
            logger.info(f"Saved: {output_file} ({len(category_data)} data points)")

        summary_file = self.output_dir / "collection_summary_all_groups.json"

        # Build global treatment mapping from group_types
        treatment_mapping = {}
        for group_name in [g.name for g in self.group_dirs]:
            info = self._get_group_info(group_name)
            treatment_id = info["treatment_id"]
            if treatment_id not in treatment_mapping:
                treatment_mapping[treatment_id] = {
                    "type": self._get_group_info(treatment_id)["group_type"],
                    "runs": [],
                }
            treatment_mapping[treatment_id]["runs"].append(group_name)

        # Sort runs within each treatment
        for tid in treatment_mapping:
            treatment_mapping[tid]["runs"] = sorted(treatment_mapping[tid]["runs"])
