...
try:
    ...
    from researcher.analysis.agent.utils.tool_registry import TOOLS  # type: ignore
...
def _invoke_tool_by_spec(
    ...
    """
    根据 TOOLS[method]['args'] 进行稳健的参数映射与调用。
    返回 (out, warnings)。
    """
    warnings: List[str] = []
    ...
    # 允许显式覆盖列名
    group_col = params.get("group_col") or group_col
    dv_col = params.get("value_col") or dv_col
    ...
    # 模式 2：*samples / *groups -> 传入多组样本
    if any(str(a).startswith("*") for a in args):
        sample_list = [samples_map[g] for g in groups if g in samples_map and len(samples_map[g]) > 0]
        if len(sample_list) < 2:
            warnings.append(
                "Fewer than two non-empty groups are available for comparison."
            )
        out = func(*sample_list)
        return out, warnings

    # 模式 3：两样本比较
    if ("sample1" in args) and ("sample2" in args):
        pair: Optional[List[str]] = None
        if isinstance(params.get("pair"), list) and len(params["pair"]) >= 2:
            pair = params["pair"][:2]
        elif params.get("group1") and params.get("group2"):
            pair = [params["group1"], params["group2"]]
        elif len(groups) >= 2:
            pair = groups[:2]
        else:
            warnings.append(
                "Pair information is missing and fewer than two groups were provided."
            )
            pair = None

        if pair is None:
            raise ValueError("The two-sample test has no usable groups.")

        s1 = samples_map.get(pair[0], [])
        s2 = samples_map.get(pair[1], [])
        out = func(s1, s2, **{k: v for k, v in params.items() if k not in ("pair", "group1", "group2", "sample1", "sample2")})
        return out, warnings
    ...
    if ("x" in args) and ("y" in args):
        ...
        if not x_col or not y_col:
            raise ValueError("The method requires x and y column names.")
        x = pd.to_numeric(df[x_col], errors="coerce").dropna().tolist()
        y = pd.to_numeric(df[y_col], errors="coerce").dropna().tolist()
        out = func(x, y, **{k: v for k, v in params.items() if k not in ("x", "y")})
        return out, warnings

    # 模式 5：单序列
    if args == ["series"]:
        series_col = params.get("series") or dv_col
        series = pd.to_numeric(df[series_col], errors="coerce").dropna().tolist()
        out = func(series, **{k: v for k, v in params.items() if k != "series"})
        return out, warnings

    # 模式 6：列联表
    if args == ["contingency_table"]:
        table = params.get("contingency_table")
        if table is None:
            raise ValueError("The method requires contingency_table.")
        out = func(table, **{k: v for k, v in params.items() if k != "contingency_table"})
        return out, warnings
    ...
    # 兜底：尝试把 df 与 params 传入（可能不适配，捕获异常）
    out = func(df, **params)
    return out, warnings
    ...
    def run(
        ...
        if request_obj is None:
            results["errors"].append(
                f"analysis_request.json is missing or invalid: {request_abs}"
            )
            return results
        ...
        for method in request_obj.get("methods", []):
            ...
            if data_obj is None:
                ...
                method_result["warnings"].append(
                    f"Data file is missing or invalid: {apply_abs}"
                )
            ...
            try:
                if method_name_req == "anova":
                    ...
                    if group_col is None:
                        ...
                        method_result["warnings"].append(
                            "Could not infer a group column for ANOVA."
                        )
                    else:
                        samples_map = _extract_group_samples(df, groups, dv_col, group_col)
                        ...
                        if len(samples) < 2:
                            method_result["status"] = "error"
                            method_result["warnings"].append(
                                "ANOVA requires at least two non-empty groups."
                            )
                        else:
                    ...
                    if group_col is None:
                        method_result["status"] = "error"
                        method_result["warnings"].append(
                            "Could not infer a group column for pairwise t-tests."
                        )
                    else:
                        samples_map = _extract_group_samples(df, groups, dv_col, group_col)
                                ...
                                pair_results.append(
                                    {
                                        "pair": [g1, g2],
                                        "status": "skipped",
                                        "reason": "The sample is empty or missing.",
                                    }
                                )
                                continue
                        ...
                        if correction == "bonferroni":
                            ...
                            method_result["warnings"].append(
                                f"Unsupported correction {correction!r}; "
                                "returning unadjusted p-values."
                            )
                    ...
                    if not formula:
                        method_result["status"] = "error"
                        method_result["warnings"].append(
                            "ols_regression requires a formula parameter."
                        )
                    else:
                        try:
                    ...
                    if method_name_req in TOOLS:
                        ...
                        method_result["status"] = "skipped"
                        method_result["warnings"].append(
                            f"Unsupported method: {method_name_req}."
                        )
            except Exception as e:
                method_result["status"] = "error"
                method_result["warnings"].append(f"Method execution failed: {e}")
        ...
        try:
            ...
            results["errors"].append(f"Failed to write the result file: {e}")
            results["meta"]["output_path"] = str(output_abs)
