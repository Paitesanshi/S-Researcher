...
from researcher.env_design.paradigm_agent import ParadigmAgent
from researcher.env_design.parameter_generator import ParameterGenerator
from researcher.env_design.paradigm_selector import ParadigmSelector
...
class Coordinator:
    ...
    def __init__(
        ...
        Args:
            ...
            research_paradigm (str, optional): Research paradigm to use.
            save_intermediate (bool, optional): Whether to save intermediate outputs.
            project_name (str, optional): Name of the research project.
            base_path (str, optional): Base path for projects (defaults to ../projects).
        """
        ...
        # Initialize paradigm selector for auto-selection
        self.paradigm_selector = ParadigmSelector(model_name)
    ...
    def run(self, input_data) -> Dict[str, Any]:
        ...
        # Auto-select paradigm if not specified or set to 'auto'
        if not self.research_paradigm or self.research_paradigm =="auto":
            logger.info("Auto-selecting research paradigm based on context...")
            self.research_paradigm = self.paradigm_selector.select_paradigm(
            ...
            logger.info(f"✓ Auto-selected paradigm: {self.research_paradigm}")
        ...
        project_root = self.base_path if self.base_path else Path(__file__).resolve().parent.parent
        project_path = Path(project_root) / self.project_name
        logger.info(f"Saving outputs to project directory: {project_path}")

        # Create output directories
        project_path.mkdir(parents=True, exist_ok=True)
        base_scenario_path= project_path / "base_scenario"
        base_scenario_path.mkdir(parents=True, exist_ok=True)
        experiment_design_path = project_path / "experiment_design"
        experiment_design_path.mkdir(parents=True, exist_ok=True)

        # === Use unified ParadigmAgent workflow for all paradigms ===
        logger.info(f"Running paradigm workflow: {self.research_paradigm}")

        paradigm_agent = ParadigmAgent(self.model_name, self.research_paradigm)

        # Build paradigm input from generated parameters
        paradigm_input = {
        ...
        # Run paradigm workflow (handles all paradigms including inductive)
        workflow_outputs = paradigm_agent.run(paradigm_input)

        # Extract outputs
        inspiration_output = workflow_outputs.get("inspiration_output", {})
        detailer_output = workflow_outputs.get("odd_output", {})

        # Save core outputs
        self._save_json(
            inspiration_output,
            str(base_scenario_path / "inspiration_output.json")
        )
        logger.info(f"✓ Saved inspiration output to: {base_scenario_path / 'inspiration_output.json'}")

        self._save_json(
            detailer_output.get("odd_protocol", {}),
            str(base_scenario_path / "odd_protocol.json")
        )
        logger.info(f"✓ Saved ODD protocol")

        # Save full workflow outputs for paradigms with experimental design
        # This includes deductive (multi-hypothesis), abductive, and legacy paradigms
        if "experiment_output" in workflow_outputs:
            # Create base_scenario and experiment_design directories
            base_scenario_path.mkdir(parents=True, exist_ok=True)
            experiment_design_path.mkdir(parents=True, exist_ok=True)

            # Save experiment configuration
            if workflow_outputs.get("experiment_output"):
                self._save_json(
                    workflow_outputs["experiment_output"],
                    str(experiment_design_path / "experiment_config.json")
                )
                logger.info(f"✓ Saved experiment configuration")

            # Save intervention specifications
            if workflow_outputs.get("intervention_output"):
                self._save_json(
                    workflow_outputs["intervention_output"],
                    str(experiment_design_path / "intervention_specifications.json")
                )
                logger.info(f"✓ Saved intervention specifications")

            # Save scenario configuration
            if workflow_outputs.get("scenario_output"):
                scenario_output = workflow_outputs["scenario_output"]

                # Save scenario_config.json to base_scenario/
                self._save_json(
                    scenario_output,
                    str(base_scenario_path / "scenario_config.json")
                )
                logger.info(f"✓ Saved scenario configuration")

        # Update scene_name from detailer output if available
        self.scene_name = detailer_output.get("scene_name", self.project_name)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        choices=[
            ...
            "deductive", "inductive", "abductive", "auto",
            # Legacy paradigms (backward compatibility)
            "theory_validation", "mechanism_discovery", "attribution_analysis", "boundary_exploration", "auto_agent"
        ],
        default="auto",
        help="Research paradigm to use (new: deductive/inductive/abductive, legacy: theory_validation/mechanism_discovery/attribution_analysis/boundary_exploration)"
    )
    parser.add_argument(
        "--scene_name",
        type=str,
