...
class ProjectConfig:
    ...
    intervention_specs_path: Optional[str]
...
class ExperimentProject:
    ...
    def __init__(self, project_config: ProjectConfig):
        ...
        if project_config.simulation_config:
            environment_config = self.config_generator.base_config.setdefault(
                "simulator", {}
            ).setdefault("environment", {})
            for key in ("mode", "max_steps", "interval", "bus_idle_timeout"):
                if key in project_config.simulation_config:
                    value = project_config.simulation_config[key]
        ...
        self.scene_path: Optional[str] = None  # Store actual environment path
    ...
    def parse_intervention_assignments(self, experiment_config: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        # Process replication_settings - auto-generate replicates for all treatment groups
        replication_settings = experimental_groups.get('replication_settings', {})
        num_replicates = replication_settings.get('num_replicates', 3)

        if num_replicates > 1:
            # For each treatment group, create (num_replicates - 1) additional replicate intervention configs
            # The first run is the original treatment group, additional runs are replicates
            for treatment_group in experimental_groups.get('treatment_groups', []):
                group_name = treatment_group['name']
                for _ in range(1, num_replicates):
                    intervention_config = {
                        'treatment_group_name': group_name,  # Use treatment group's intervention spec
                        'timing': 'pre_simulation',
                        'llm_enhancement': False
                    }
                    intervention_configs.append(intervention_config)

        # Also process legacy manually defined replicates for backward compatibility
        for replicate in experimental_groups.get('replicates', []):
            base_treatment = replicate.get('base_treatment')
            if base_treatment:
                intervention_config = {
                    'treatment_group_name': base_treatment,
                    'timing': 'pre_simulation',
                    'llm_enhancement': False
                }
                intervention_configs.append(intervention_config)

        logger.info(f"Parsed {len(intervention_configs)} intervention assignments (including {num_replicates} replicates per group)")
        return intervention_configs
    
    def load_base_profiles(self, scene_path: Union[str, Path]) -> Dict[str, List[AgentProfile]]:
        """
        ...
        Args:
            scene_path: Path to the environment directory
            
        Returns:
            Dictionary mapping agent types to lists of profiles
        """
        env_path = Path(scene_path)
        ...
        self.scene_path = str(scene_path)  # Store the actual environment path
    ...
    def design_experiment(
        ...
        num_replicates = replication_settings.get('num_replicates', 3)
        base_seed = replication_settings.get('base_seed', 10001)
        seed_strategy = replication_settings.get('seed_strategy', 'sequential')
        
        # Create observation runs (for inductive paradigm)
        if 'observation_runs' in experiment_design:
            obs_spec = experiment_design['observation_runs']
            obs_seed = obs_spec.get('parameters', {}).get('random_seed', base_seed)
            
            # Create observation run groups with replicates
            for rep_idx in range(num_replicates):
                # Generate seed based on strategy
                if seed_strategy == 'sequential':
                    replicate_seed = obs_seed + rep_idx * 1000
                elif seed_strategy == 'random':
                    import random
                    replicate_seed = random.randint(1, 999999)
                else:  # fixed or other
                    replicate_seed = obs_seed + rep_idx
                
                group_suffix = f"_rep_{rep_idx}" if rep_idx > 0 else ""
                obs_group = ExperimentGroupConfig(
                    group_id=f"{obs_spec['name']}{group_suffix}",
                    group_type='observation',
                    description=obs_spec['description'] if rep_idx == 0 else f"Replicate {rep_idx} of {obs_spec['description']}",
                    profile_modifications=[],
                    custom_config_overrides=obs_spec.get('parameters', {}),
                    random_seed=replicate_seed
                )
                groups.append(obs_group)
        ...
        # Create treatment groups and their replicates
        if 'treatment_groups' in experiment_design:
            for treatment_spec in experiment_design['treatment_groups']:
                ...
                treatment_seed = treatment_spec.get('parameters', {}).get('random_seed', base_seed)

                # Create the original treatment group (rep_0)
                treatment_group = ExperimentGroupConfig(
                    ...
                    random_seed=treatment_seed
                )
                groups.append(treatment_group)

                # Auto-generate replicate groups if num_replicates > 1
                if num_replicates > 1:
                    for rep_idx in range(1, num_replicates):
                        # Generate seed based on strategy
                        if seed_strategy == 'sequential':
                            replicate_seed = treatment_seed + rep_idx * 1000
                        elif seed_strategy == 'random':
                            import random
                            replicate_seed = random.randint(1, 999999)
                        else:  # fixed or other
                            replicate_seed = treatment_seed + rep_idx

                        replicate_group = ExperimentGroupConfig(
                            group_id=f"{treatment_spec['name']}_rep_{rep_idx}",
                            group_type='replicate',
                            description=f"Replicate {rep_idx} of {treatment_spec['description']}",
                            profile_modifications=[intervention_id],
                            random_seed=replicate_seed
                        )
        ...
        logger.info(f"Designed experiment with {len(groups)} groups (num_replicates={num_replicates})")
        return groups
    ...
    def apply_interventions(
        ...
        Args:
            ...
            scene_path: Path to base environment for copying control group profiles
        ...
        # Apply interventions to each experiment group based on its profile_modifications
        for group_config in experiment_groups:
            if group_config.group_type in ('treatment', 'replicate') and group_config.profile_modifications:
                # Get the treatment group name from profile_modifications
                treatment_group_name = group_config.profile_modifications[0]
                ...
                processed_groups.add(group_config.group_id)

                # Save modified profiles
                if intervention_result.success:
                    self._save_group_profiles(group_config, intervention_result)
                    logger.info(f"Applied intervention '{treatment_group_name}' to group '{group_config.group_id}'")

        # Handle control groups and other groups without interventions
        for group_config in experiment_groups:
            if group_config.group_id not in processed_groups:
                # This is a control group or group without interventions
                self._save_control_group_profiles(group_config, scene_path)
    ...
    def _save_group_profiles(
        ...
        env_path = Path(self.scene_path) if self.scene_path else Path(self.config.base_environment)
    ...
    def generate_configurations(
        ...
        for group_config in experiment_groups:
            ...
            config_file_path = self.config_generator.generate_single_experiment_config(
                ...
                environment_name=self.scene_path,
    ...
    def run_full_workflow_from_config(
        ...
        Args:
            scene_path: Path to base environment
            experiment_config_path: Path to experiment configuration file
    ...
    def _execute_common_workflow(
        ...
        try:
            ...
            base_profiles = self.load_base_profiles(scene_path)
            ...
            intervention_results = self.apply_interventions(experiment_groups, intervention_configs, scene_path)
