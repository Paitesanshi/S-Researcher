...
class InterventionEngine:
    ...
    def load_intervention_specifications(self, spec_file: Union[str, Path, None]) -> None:
        """
        Load intervention specifications from JSON file.
        
        Args:
            spec_file: Path to intervention specifications file, or None to skip loading
        """
        # If spec_file is None, skip loading (e.g., for inductive paradigm)
        if spec_file is None:
            logger.info("No intervention specifications provided - skipping intervention loading")
            return
