    ...
    def create_experiment_run_configs(
        ...
        for group_type, group_data in experimental_groups.items():
            if group_type == 'control_group':
                ...
                config_path = self._generate_single_config(group_config, output_path)
                ...
                for treatment in group_data:
                    ...
                    treatment_seed = treatment.get('parameters', {}).get('random_seed', base_seed)

                    # Create the original treatment group
                    group_config = ExperimentGroupConfig(
                    ...
                    # Auto-generate replicate configs if num_replicates > 1
                    if num_replicates > 1:
                        for rep_idx in range(1, num_replicates):
                            ...
                            replicate_config = ExperimentGroupConfig(
                                group_id=f"{treatment['name']}_rep_{rep_idx}",
                                group_type='replicate',
                                description=f"Replicate {rep_idx} of {treatment['description']}",
                                profile_modifications=[intervention_id],
                                random_seed=replicate_seed
                            )
                            config_path = self._generate_single_config(replicate_config, output_path)
                            run_configs[replicate_config.group_id] = config_path

            elif group_type == 'replicates':
                # Legacy support for manually defined replicates
                ...
                for agent_type, profile_path in modified_profile_paths.items():
                    ...
                    try:
                        with open(profile_path, 'r', encoding='utf-8') as f:
                            ...
                            if "agent" in self.base_config and "profile" in self.base_config['agent'] and agent_type in self.base_config['agent']['profile'] and 'count' in self.base_config['agent']['profile'][agent_type]:
                                experiment_config['agent']['profile'][agent_type]['count'] = self.base_config['agent']['profile'][agent_type]['count']
                            else:
