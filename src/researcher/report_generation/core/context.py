...
class ReportContext:
    ...
    def _load_core_data(self, project_root: Path):
        ...
        # Load analysis data from multiple sources and combine them
        analysis_dir = project_root / "analysis"
        combined_analysis = {}

        # Source 1: data_analysis.json (legacy)
        ...
        # Source 2: figure_analysis.json (legacy) or figures/fig_explanations.json (new)
        figure_analysis_candidates = [
            analysis_dir / "figures" / "fig_explanations.json",  # stage2 new output
            analysis_dir / "figure_analysis.json",  # legacy path
        ]
        for figure_path in figure_analysis_candidates:
            if figure_path.exists():
                figure_analysis = self._load_json_safe(figure_path)
                if figure_analysis:
                    combined_analysis["figure_analysis"] = figure_analysis
                    logger.info(f"Loaded figure analysis from: {figure_path.name}")
                    break

        # Source 3: stage3_analysis_results.json
        stage3_path = analysis_dir / "data" / "stage3_analysis_results.json"
        if stage3_path.exists():
            stage3_data = self._load_json_safe(stage3_path)
            if stage3_data:
                combined_analysis["stage3_analysis"] = stage3_data
                logger.info(f"Loaded stage3 analysis from: {stage3_path.name}")

        # Source 4: analysis_report.json (comprehensive report)
        report_path = analysis_dir / "analysis_report.json"
        if report_path.exists():
            report_data = self._load_json_safe(report_path)
            if report_data:
                combined_analysis["analysis_report"] = report_data
                logger.info(f"Loaded analysis report from: {report_path.name}")

        # If we have any combined analysis, use it
        if combined_analysis:
            self.analysis_data = json.dumps(combined_analysis, indent=2, ensure_ascii=False)
        else:
            # Fallback to old analysis candidates (txt files)
            analysis_candidates = [
                project_root / "analysis" / "analysis_report.txt",
                project_root / "analysis_report.json",
                project_root / "analysis_report.txt"
            ]
        ...
        # Load metrics data from various possible locations
        # Priority: fig_explanations.json (from stage2) > figure_analysis.json > metrics.json
        metrics_candidates = [
            project_root / "analysis" / "figures" / "fig_explanations.json",  # stage2 output
        ...
        for candidate in metrics_candidates:
            if candidate.exists():
                ...
                if self.metrics_data:
                    logger.info(f"Loaded metrics data from: {candidate.name}")
                    break
    ...
    def _generate_outline_from_reference_pdf(self, pdf_path: Path, model_config_name: Optional[str] = None) -> Optional[str]:
        ...
        try:
            ...
            def extract_pdf_to_markdown(path: str) -> str:
...
Please provide a structured outline that includes both the content structure and writing guidance for each section."""
    ...
    def generate_project_specific_outline(self, model_config_name: Optional[str] = None) -> str:
        ...
        try:
            ...
            system_prompt = """You are an expert academic writing assistant. Generate a detailed,
project-specific writing outline from the reference-paper structure and the
current project's evidence.

The outline must:
1. Follow a standard academic-paper structure.
2. Address the project's research question and available data.
3. Give concrete guidance for every section.
4. Recommend where figures and tables should be cited.
5. Use LaTeX syntax rather than Markdown.
6. Be written entirely in English."""

            # Prepare project context
            project_info = f"""
Project information:
- Research question: {self.research_question}
- Research paradigm: {self.paradigm}
- Scenario description: {self.scenario_description[:200]}...
- Experimental data available: {'yes' if self.experiment_config else 'no'}
- Analysis data available: {'yes' if self.analysis_data else 'no'}
- Figures available: {'yes' if self.image_paths else 'no'} ({len(self.image_paths)} figures)
"""

            reference_outline = (
                self.outline_template
                if self.outline_template
                else "No reference outline is available."
            )

            user_prompt = f"""Generate a detailed, project-specific writing outline from the
...
The outline must cover:
1. Abstract guidance.
2. Introduction structure.
3. Methodology.
4. Results and analysis, including concrete data-presentation and table suggestions.
5. Discussion and conclusion.
6. Figure and table citation suggestions in LaTeX.
7. Reference integration.

Match the outline to the research question and available evidence. Use LaTeX
syntax and write only in English."""
