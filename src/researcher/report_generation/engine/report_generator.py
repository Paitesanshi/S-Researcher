...
class ReportGenerator:
    ...
    def generate(self, context: ReportContext) -> str:
        ...
        # LLM-authored sections may contain plausible-looking citation keys that
        # were not returned by the literature search. Keep only keys that will
        # actually be written to report.bib so the deliverable has no dangling
        # LaTeX citations when a search service is unavailable or rate-limited.
        final_report = self._sanitize_citations(
            final_report, set(context.citation_entries)
        )
    ...
    @staticmethod
    def _sanitize_citations(content: str, valid_keys: set[str]) -> str:
        """Remove unknown keys from LaTeX citation commands."""
        removed_keys = set()

        # Repair a common model formatting error such as
        # ``\cite{cite{ref4}}`` or ``\cite{\cite{ref4}}`` before checking keys.
        nested_citation = re.compile(
            r"\\cite[a-zA-Z]*\{(?:\\)?cite[a-zA-Z]*\{([^{}]+)\}\}"
        )
        while nested_citation.search(content):
            content = nested_citation.sub(r"\\cite{\1}", content)
        ...
        def replace(match: re.Match) -> str:
            ...
            known = [key for key in requested if key in valid_keys]
            removed_keys.update(set(requested) - set(known))
            return f"\\{command}{{{','.join(known)}}}" if known else ""

        sanitized = re.sub(
            r"\\(?P<command>cite[a-zA-Z]*)\{(?P<keys>[^{}]+)\}",
            replace,
            content,
        )
        if removed_keys:
            logger.warning(
                "Removed {} citation keys missing from report.bib",
                len(removed_keys),
            )
        return sanitized
    ...
    def _load_references(self, context: ReportContext):
        ...
        if self.config.outline_template_path:
            context._load_outline_template(self.config.outline_template_path)
    ...
    def _get_document_preamble(self) -> str:
        ...
        if self.config.language == "zh":
            # Use the TeX Live bundled font set instead of platform-specific
            # macOS/Windows fonts, which may not be installed on another host.
            preamble.append("\\usepackage[fontset=fandol]{ctex}")
