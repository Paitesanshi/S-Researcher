...
class ScenarioCreationPipeline:
    ...
    def __init__(
        ...
        Args:
            ...
            project_name: Project name under the default projects directory
            project_path: Explicit project directory; overrides project_name resolution
        ...
        self.project_path = (
            ...
            if project_path
            else self.base_path / "projects" / self.project_name
        )
        ...
        with open(self.project_path/'base_scenario'/'odd_protocol.json','r',encoding='utf-8') as f:
            self.state["odd_protocol"] = json.load(f)
        # Initialize agents
        self.model = None
    ...
    def load_scene_info(self) -> Dict[str, Any]:
        ...
        if self.state['odd_protocol']:
            scene_info["odd_protocol"] = self.state['odd_protocol']
    ...
    def restore_completed_steps(self) -> set:
        """Restore pipeline state from complete artifacts left by an earlier run."""
        completed = set()
        scene_info = self.load_scene_info()

        agent_types = scene_info.get("agent_types", {})
        if isinstance(agent_types, dict) and agent_types:
            self.state["agent_types"] = list(agent_types)
            completed.add("Generate Agent Types")

        workflow_files = {
            "actions": self.env_path / "actions.json",
            "events": self.env_path / "events.json",
            "system_data_model": self.env_path / "system_data_model.json",
        }
        if all(path.exists() for path in workflow_files.values()):
            for state_key, path in workflow_files.items():
                with open(path, "r", encoding="utf-8") as handle:
                    self.state[state_key] = json.load(handle)
            completed.add("Generate Workflow")

        metrics_module = self.env_path / "code" / "metrics" / "metrics.py"
        if scene_info.get("metrics") and metrics_module.exists():
            self.state["metrics"] = scene_info["metrics"]
            completed.add("Generate Metrics")

        env_data_path = self.env_path / "env_data.json"
        if env_data_path.exists():
            with open(env_data_path, "r", encoding="utf-8") as handle:
                self.state["env_data"] = json.load(handle)
            completed.add("Generate Environment Data")

        code_dir = self.env_path / "code"
        if (
            self.state["agent_types"]
            and (code_dir / "events.py").exists()
            and all((code_dir / f"{agent_type}.py").exists()
                    for agent_type in self.state["agent_types"])
        ):
            completed.add("Generate Code")

        schema_dir = self.env_path / "profile" / "schema"
        if (
            self.state["agent_types"]
            and all((schema_dir / f"{agent_type}.json").exists()
                    for agent_type in self.state["agent_types"])
        ):
            completed.add("Save Profile Schemas")

        relationship_schema_path = schema_dir / "Relationship.json"
        if relationship_schema_path.exists():
            with open(relationship_schema_path, "r", encoding="utf-8") as handle:
                self.state["relationship_schema"] = json.load(handle)
            completed.add("Generate Relationship Schema")

        topology_path = schema_dir / "RelationshipTopology.json"
        if topology_path.exists():
            with open(topology_path, "r", encoding="utf-8") as handle:
                self.state["topology_config"] = json.load(handle)
            completed.add("Generate Topology Configuration")

        if completed:
            logger.info(
                "Restored completed scenario steps from disk: {}",
    ...
    def generate_profile_schemas(self) -> bool:
    ...
    def debug_environment(self) -> bool:
        ...
        try:
            ...
            # A reviewer-facing command-line run must not depend on a private
            # Docker image.  Perform deterministic local validation by default;
            # the original LLM-assisted Docker repair loop remains opt-in.
            if os.environ.get("ONESIM_ENABLE_DOCKER_DEBUG", "0") != "1":
                return self._validate_generated_environment()
    ...
    def _validate_generated_environment(self) -> bool:
        """Validate generated JSON and Python files without external services."""
        errors = []

        required_files = [
            self.env_path / "scene_info.json",
            self.env_path / "actions.json",
            self.env_path / "events.json",
            self.env_path / "env_data.json",
            self.env_path / "code" / "SimEnv.py",
        ]
        for path in required_files:
            if not path.exists():
                errors.append(f"missing required file: {path}")

        for path in self.env_path.rglob("*.json"):
            try:
        ...
        for path in self.env_path.rglob("*.py"):
            try:
        ...
        if errors:
            for error in errors:
                logger.error(error)
            logger.error(
                f"Local validation failed with {len(errors)} issue(s). "
                "Set ONESIM_ENABLE_DOCKER_DEBUG=1 to enable the optional repair loop."
            )
            return False

        logger.info("✓ Generated environment passed local JSON/Python validation")
        return True
        ...
        try:
            ...
            if hasattr(result, 'success'):
                if result.success:
                    ...
                    output_message += "Final status: validation succeeded; the environment runs normally"
                    ...
                    self.state["statistics"]["debug_results"] = {
                        ...
                        "final_status": "Validation succeeded; the environment runs normally"
                    }
                    return True
                else:
                    ...
                    output_message += "Final status: validation failed"
            ...
            # Handle dictionary format (fallback compatibility)
                ...
                if result.get("success"):
                    ...
                    output_message += (
                        f"Final status: {result.get('final_status', 'Validation failed')}"
                    )
                    logger.error(output_message)
    ...
    def run_full_pipeline(self) -> bool:
        ...
        try:
            ...
            completed_steps = self.restore_completed_steps()
            ...
            steps = [
                ...
                ("Save Profile Schemas", self.generate_profile_schemas),
            ...
            for step_name, step_func in steps:
                ...
                if step_name in completed_steps:
                    logger.info(f"✓ Restored completed step: {step_name}")
                    continue
                if not step_func():
...
Examples:
  ...
  python src/researcher/scenario_creation.py --config config/my_scenario.json
    ...
    parser.add_argument(
        "--project_path",
        type=str,
        help="Explicit project directory (overrides base_path/projects/project_name)"
    )
...
def main():
    ...
    try:
        ...
        if args.config:
            ...
            project_path = config.get("project_path", args.project_path)
        else:
            topic = args.topic
