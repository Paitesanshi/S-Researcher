...
class AgentConfig:
    ...
    # NOTE: memory can be explicitly disabled by setting it to None in config.
    # It must therefore be Optional and all serializers/loaders must handle None safely.
...
class InterventionConfig:
    ...
    enabled: bool = False
    intervention_specs_path: Optional[str] = None
    
    def to_dict(self) -> Dict[str, Any]:
        """Convert instance to a dictionary for JSON serialization"""
        return {
            "enabled": self.enabled,
            "intervention_specs_path": self.intervention_specs_path
        }
    ...
    def _apply_loaded_config(self, loaded_config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if "agent" in loaded_config:
                ...
                if "relationship_path" in agent_config:
                    self.agent_config.relationship_path = agent_config[
                        "relationship_path"
                    ]
                ...
                if "memory" in agent_config:
                    ...
                    if not memory_config:
                        # Explicitly disable memory system when config provides null/empty.
                        self.agent_config.memory = None
                    else:
                        # If memory was previously disabled (None) but is enabled now,
                        # recreate the default memory config to avoid NoneType crashes.
                        if self.agent_config.memory is None:
                            self.agent_config.memory = AgentMemoryConfig()
                ...
                # If "memory" is not present, do NOT clear existing/default memory config.
                # This keeps config loading idempotent across partial updates and restarts.
            ...
            # Handle intervention configuration
            if "intervention_config" in loaded_config:
                intervention_config = loaded_config["intervention_config"]
                self._apply_config(self.intervention_config, intervention_config)
...
def reset_config() -> OneSimConfig:
    """
    Reset the global config instance to a fresh default OneSimConfig.

    This is useful when switching environments/scenarios within the same process
    to avoid stale values carrying over from a previously loaded config.
    """
    global _config_instance
    _config_instance = OneSimConfig()
    return _config_instance
