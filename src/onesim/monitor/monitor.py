...
class MonitorManager:
    ...
    def setup(self, env: Any):
        ...
        self.env = env
        # 同时设置问卷管理器的环境
        self.questionnaire_manager.setup(env)
    ...
    async def setup_metrics(env: Any):
        ...
        # # 生成并加载问卷
        # try:
        #     await monitor_manager.setup_questionnaires(env)
        # except Exception as e:
        #     logger.error(f"设置问卷失败: {e}")
    ...
    async def update_all_metrics(self, env: Any = None):
        """
        立即更新所有指标（用于在特定时间点如step结束时同步更新）
        
        Args:
            env: 环境对象，如果不提供则使用已设置的环境
        """
        if not env and not self.env:
            logger.error(f"无法更新指标：未提供环境对象")
            return
        ...
        if not metric_names:
            logger.debug("没有注册的指标需要更新")
            return
        
        logger.info(f"开始同步更新 {len(metric_names)} 个指标")
        
        # 逐个更新指标（已经在update_metric中有锁保护）
        for metric_name in metric_names:
            try:
                await self.update_metric(metric_name, env)
            except Exception as e:
                logger.error(f"更新指标 {metric_name} 时出错: {e}")
        
        logger.info(f"完成 {len(metric_names)} 个指标的同步更新")
    ...
    def plot_metrics(self, metrics_data: Dict[str, Any], save_dir: str, round_num: Optional[int] = None) -> None:
        ...
        metrics_file = os.path.join(save_dir, f'general_metrics.json')
    ...
    def plot_registered_metrics(self, save_dir: str, round_num: Optional[int] = None) -> None:
        ...
        for metric_name, result in self.results.items():
            ...
            try:
                ...
                filename = f"{metric_name}{round_info}.png"
                plt.savefig(os.path.join(metric_dir, filename), dpi=300, bbox_inches='tight')
                
                # 保存原始数据为JSON
                data_filename = f"{metric_name}{round_info}.json"
    ...
    async def setup_questionnaires(self, env: Any):
        """
        为场景生成和加载问卷
        ...
        """
        env_path = env.env_path
        questionnaires_dir = os.path.join(env_path, "questionnaires")
        os.makedirs(questionnaires_dir, exist_ok=True)

        # 检查是否已有问卷文件
        questionnaires_index_path = os.path.join(questionnaires_dir, "questionnaires.json")

        if os.path.exists(questionnaires_index_path):
            # 加载现有问卷
            logger.info("加载现有问卷...")
            await self.load_questionnaires(questionnaires_dir)
        else:
            # 生成新问卷
            logger.info("生成新问卷...")
            await self.generate_and_save_questionnaires(env, questionnaires_dir)

    async def generate_and_save_questionnaires(self, env: Any, output_dir: str):
        """
        使用QuestionnaireAgent生成问卷并保存到文件
        ...
        """
        try:
            # 导入QuestionnaireAgent
            from onesim.agent import QuestionnaireAgent
            from onesim.config import get_config

            # 获取场景信息
            scene_info_path = os.path.join(env.env_path, "scene_info.json")
            if not os.path.exists(scene_info_path):
                logger.warning("未找到scene_info.json,跳过问卷生成")
                return

            with open(scene_info_path, 'r', encoding='utf-8') as f:
                scene_info = json.load(f)

            scenario_description = scene_info.get("description", "")

            # 获取Agent类型
            agent_types = []
            if hasattr(env, 'agents') and env.agents:
                agent_types = list(set(agent.agent_type for agent in env.agents))

            if not agent_types:
                logger.warning("未找到Agent类型,跳过问卷生成")
                return

            # 获取Agent画像信息 (如果有)
            agent_profiles = {}
            profile_dir = os.path.join(env.env_path, "profile")
            if os.path.exists(profile_dir):
                for agent_type in agent_types:
                    profile_schema_path = os.path.join(profile_dir, f"{agent_type}_profile_schema.json")
                    if os.path.exists(profile_schema_path):
                        with open(profile_schema_path, 'r', encoding='utf-8') as f:
                            agent_profiles[agent_type] = json.load(f)

            # 创建QuestionnaireAgent
            config = get_config()
            model_config_name = config.model_config.chat_configs[0].get("config_name", "default") if config.model_config.chat_configs else "default"

            questionnaire_agent = QuestionnaireAgent(model_config_name=model_config_name)

            # 生成问卷 (默认生成2个问卷)
            questionnaires = questionnaire_agent.generate_questionnaires(
            ...
            if not questionnaires:
                logger.warning("问卷生成失败")
                return

            # 格式化问卷
            formatted_questionnaires = questionnaire_agent.format_questionnaires_for_export(questionnaires)

            # 保存问卷
            questionnaires_index = {
                "questionnaires": [],
                "created_at": time.time(),
                "scenario": scenario_description[:200]  # 保存场景描述摘要
            }

            for q_def in formatted_questionnaires:
                # 创建Questionnaire对象
                from .questionnaire import Question, QuestionType

                questions = []
                for q in q_def.get("questions", []):
                    question = Question(
                        id=q.get("id", ""),
                        text=q.get("text", ""),
                        question_type=QuestionType(q.get("question_type", "text")),
                        options=q.get("options"),
                        scale_range=tuple(q.get("scale_range")) if q.get("scale_range") else None,
                ...
                questionnaire = Questionnaire(
                    ...
                    questions=questions,
                    target_agent_types=q_def.get("target_agent_types", []),
                    metadata=q_def.get("metadata", {})
                )

                # 保存单个问卷为JSON文件
                questionnaire_file = os.path.join(output_dir, f"{questionnaire.id}.json")
                questionnaire.to_json(questionnaire_file)
                logger.info(f"已保存问卷: {questionnaire_file}")

                # 注册问卷到管理器
                self.questionnaire_manager.register_questionnaire(questionnaire)

                # 添加到索引
                questionnaires_index["questionnaires"].append({
                    "id": questionnaire.id,
                    "title": questionnaire.title,
                    "file": f"{questionnaire.id}.json",
                    "target_agent_types": questionnaire.target_agent_types
                })

            # 保存问卷索引
            index_path = os.path.join(output_dir, "questionnaires.json")
            with open(index_path, 'w', encoding='utf-8') as f:
                json.dump(questionnaires_index, f, indent=2, ensure_ascii=False)

            logger.info(f"成功生成并保存 {len(formatted_questionnaires)} 个问卷到 {output_dir}")

        except Exception as e:
            logger.error(f"生成问卷时出错: {e}", exc_info=True)

    async def load_questionnaires(self, questionnaires_dir: str):
        """
        从目录加载问卷

        Args:
            questionnaires_dir: 问卷目录
        """
        try:
            index_path = os.path.join(questionnaires_dir, "questionnaires.json")

            if not os.path.exists(index_path):
                logger.warning(f"未找到问卷索引文件: {index_path}")
                return

            with open(index_path, 'r', encoding='utf-8') as f:
                index_data = json.load(f)

            for q_info in index_data.get("questionnaires", []):
                questionnaire_file = os.path.join(questionnaires_dir, q_info.get("file", ""))

                if os.path.exists(questionnaire_file):
                    questionnaire = Questionnaire.from_json(questionnaire_file)
                    self.questionnaire_manager.register_questionnaire(questionnaire)
                    logger.info(f"加载问卷: {questionnaire.title}")
                else:
                    logger.warning(f"问卷文件不存在: {questionnaire_file}")

            logger.info(f"从 {questionnaires_dir} 加载了 {len(index_data.get('questionnaires', []))} 个问卷")

        except Exception as e:
            logger.error(f"加载问卷时出错: {e}", exc_info=True)
