...
class CircuitBreaker:
    ...
    def __init__(self, failure_threshold=50, recovery_timeout=30.0, half_open_success=5):
...
class ConnectionShard:
                ...
                try:
                    ...
                    if key in self.channels:
                        ...
                        # 如果通道处于IDLE状态且最近使用过，尝试激活它
                            ...
                            # 尝试等待通道就绪 - 但使用更短的超时
                            try:
                                is_ready = await self._wait_for_ready_state(channel, timeout=30.0)
                                if is_ready:
                                    # 通道现在已准备好
                                    self.channels[key] = (channel, time.time(), use_count + 1)
                                    self.known_good_connections.add(key)
    ...
    def _create_channel(self, address: str, port: int, cache: bool = True) -> grpc.aio.Channel:
        ...
        # 创建新通道配置 - 优化以提高稳定性
        options = [
            ('grpc.keepalive_time_ms', 60000),           # 每10秒发送一次keepalive
            ('grpc.keepalive_timeout_ms', 20000),         # 5秒等待keepalive响应
    ...
    async def with_stub(self, address: str, port: int, stub_type, func, *args, **kwargs):
        ...
        while retry_count < max_retries:
            try:
                ...
                # 设置超时，避免长时间阻塞
                timeout = 3600.0  # 1小时超时,适应大规模仿真场景(10k+ agents)
...
class ShardedConnectionManager:
    ...
    def __init__(self):
        ...
        # 配置
        self.shard_count = 128 # 分片数量
        self.semaphore_size = 50 # 每个分片的信号量大小
        self.lock_timeout = 60  # 锁超时时间 (Reduced from 30.0)
        self.max_idle_time = 3600.0  # 空闲通道超时时间 (10 minutes)
