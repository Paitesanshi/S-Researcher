...
from onesim import planning
from onesim.config import AgentMemoryConfig
...
class WorkerNode(Node):
    ...
    def __init__(self, node_id: str, config: Optional[Dict[str, Any]] = None):
        ...
        self.heartbeat_max_interval = config.get("heartbeat_max_interval", 900)
    ...
    def load_memory(self, memory_config: AgentMemoryConfig, model_config_name: str) -> MemoryStrategy:
        ...
        # Handle both dict and AgentMemoryConfig object
        # (dict comes from gRPC/JSON serialization in distributed mode)
        if isinstance(memory_config, dict):
            strategy_class_name = memory_config.get("strategy", "ListStrategy")
        else:
            strategy_class_name = memory_config.strategy
        try:
            # Load memory module and class
    ...
    def create_local_agents(self, agent_configs: List[Dict]) -> None:
        ...
        for agent_type, configs in configs_by_type.items():
            ...
            for config in tqdm(configs):
                ...
                planning_config = config.get("planning_config",None)
                planning_instance = self.load_planning(planning_config, self.model_config_name,config["sys_prompt"])
