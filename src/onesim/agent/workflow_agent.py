    ...
    def extract_workflow(self, description: str, agent_types: list,last_workflow:str=None, issues: Optional[list] = None,suggestions: Optional[list] = None) -> dict:
            ...
            try:
                ...
                for agent in agents:
                    ...
                    # EnvAgent.start (ID 0) and EnvAgent.terminate (ID -1) are
                    # framework-owned nodes. Some chat models repeat them in the
                    # normal agents list despite the schema using workflow_metadata
                    # for starts and terminal-target events for termination. Treating
                    # those repetitions as regular actions creates duplicate,
                    # disconnected nodes that can never pass structural validation.
                ...
                # A StartEvent is only valid for a root action. Models sometimes
                # emit StartEvents for downstream actions as well as the regular
                # business events that trigger them. Those duplicate starts are
                # not registered by generated downstream agents and bypass the
                # intended event order, so remove them deterministically.
                regular_event_targets = {
                    event["to_action_id"]
                    for event_id, event in self.events.items()
                    if event_id >= 0 and event.get("to_action_id") is not None
                }
                redundant_start_ids = [
                    event_id
                    for event_id, event in self.events.items()
                    if event_id < 0
                    and event.get("to_action_id") in regular_event_targets
                ]
                for event_id in redundant_start_ids:
                    removed_event = self.events.pop(event_id)
                    logger.info(
                        "Removed redundant StartEvent for downstream action {}.{}",
                        removed_event["to_agent_type"],
                        removed_event["to_action_name"],
                    )
