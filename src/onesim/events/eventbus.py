...
class EventBus:
    ...
    def stop(self):
        """Stop the event bus and clear all state for clean restart."""
        logger.info("Stopping EventBus and clearing state")
        self._running = False
        self._paused = False
        ...
        # Clear lock registry and requests
        self._lock_registry.clear()
        self._lock_requests.clear()

        # Clear event tracking data
        self._event_sequence.clear()
        self._event_flows.clear()
        self._event_to_flow.clear()
        self._agent_types.clear()

        logger.info("EventBus stopped and state cleared")
