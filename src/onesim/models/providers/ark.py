...
class ArkChatAdapter(ModelAdapterBase):
    ...
    def __call__(
        ...
        **kwargs
        ...
        extra_body = call_args.pop("extra_body", {}) or {}
        thinking_val = {"type": "enabled"} if self._requires_thinking else {"type": "disabled"}
        extra_body.setdefault("thinking", thinking_val)
        ...
        try:
            ...
            if self._is_thinking_required_error(e) and extra_body.get("thinking", {}).get("type") != "enabled":
                logger.info(f"Model '{self.model_name}' requires thinking enabled, retrying")
                self._requires_thinking = True
                call_args["extra_body"]["thinking"] = {"type": "enabled"}
                try:
                    resp = self.client.chat.completions.create(
                        model=self.model_name,
                        messages=messages,
                    ...
                    logger.error(f"Error calling Ark API: {e2}")
                    raise
            else:
    ...
    def _is_thinking_required_error(exc: Exception) -> bool:
        """Check if the error means the model requires thinking enabled."""
