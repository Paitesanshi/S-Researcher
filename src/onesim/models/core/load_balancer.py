...
# HTTP status codes that indicate a permanent model misconfiguration.
# These should NOT trigger failover retries — the model itself is broken.
_NON_RETRYABLE_STATUS_CODES: Set[int] = {400, 401, 403, 404, 422}
...
class LoadBalancer(ModelAdapterBase):
    ...
    def _is_non_retryable(exc: Exception) -> bool:
        """Return True if the exception indicates a permanent error (e.g. 400)."""
        status = getattr(exc, "status_code", None)
        if not isinstance(status, int):
            resp = getattr(exc, "response", None)
            if resp is not None:
                status = getattr(resp, "status_code", getattr(resp, "status", None))
        return isinstance(status, int) and status in _NON_RETRYABLE_STATUS_CODES
    ...
    def __call__(self, *args, **kwargs) -> ModelResponse:
        ...
        # Try up to max_retries iterations, but also ensure every model gets a chance
        max_iterations = max(self.max_retries, len(self._model_instances))
        for _ in range(max_iterations):
            ...
            try:
                ...
                if self._is_non_retryable(e):
                    logger.warning(f"Model '{model.config_name}' returned non-retryable error, skipping")
                continue

        raise Exception(f"All models failed after trying {len(tried_models)} models. Last error: {str(last_error)}")
