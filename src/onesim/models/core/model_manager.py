...
from .retry import RetryingModelAdapter, RetryConfig
...
class ModelManager:
    ...
    def _resolve_env_placeholders(self, value: Any) -> Any:
        """Resolve ${ENV_VAR} placeholders in model config values."""
        ...
        if isinstance(value, str):
            if value.startswith("${") and value.endswith("}"):
                inner = value[2:-1]
                name, default = (inner.split(":-", 1) + [""])[:2] if ":-" in inner else (inner, "")
    ...
    def load_model_configs(
        ...
        for raw_config in config_list:
            config = self._resolve_env_placeholders(raw_config)
    ...
    def get_model(self, config_name: Optional[str] = None, model_type: Optional[str] = None, model_name: Optional[str] = None) -> ModelAdapterBase:
        ...
        if config_name is not None:
            ...
            if config_name in self.model_configs:
                ...
                return self._maybe_wrap_with_retry(model, config)
    ...
    def _maybe_wrap_with_retry(
        self, model: ModelAdapterBase, config: Dict[str, Any]
    ) -> ModelAdapterBase:
        """
        Wrap a model adapter with retry logic if configured.

        Configuration format (optional):
        {
          "retry": {
            "enabled": true,
            "max_attempts": 5,
            ...
            "jitter_ratio": 0.1,
            "rate_limit_min_delay_s": 2.0,
            "retryable_status_codes": [429, 500, 502, 503, 504]
          }
        }
        """
        retry_cfg_dict = config.get("retry")
        if not retry_cfg_dict:
            # Default: enable lightweight retries for transient errors.
            retry_cfg = RetryConfig()
        elif isinstance(retry_cfg_dict, dict):
            retry_cfg = RetryConfig(
                enabled=bool(retry_cfg_dict.get("enabled", True)),
                max_attempts=int(retry_cfg_dict.get("max_attempts", 5)),
                initial_delay_s=float(retry_cfg_dict.get("initial_delay_s", 0.5)),
                max_delay_s=float(retry_cfg_dict.get("max_delay_s", 30.0)),
                backoff_multiplier=float(retry_cfg_dict.get("backoff_multiplier", 2.0)),
                jitter_ratio=float(retry_cfg_dict.get("jitter_ratio", 0.1)),
                rate_limit_min_delay_s=float(retry_cfg_dict.get("rate_limit_min_delay_s", 2.0)),
                ...
                respect_retry_after=bool(
                    retry_cfg_dict.get("respect_retry_after", True)
                ),
                log_retries=bool(retry_cfg_dict.get("log_retries", True)),
            )
        else:
            logger.warning(
                "Invalid retry config for model '{}': expected dict, got {}. Using defaults.",
        ...
        if isinstance(model, RetryingModelAdapter):
            return model

        return RetryingModelAdapter(model, retry_cfg)
