...
class JsonBlockParser(ParserBase):
    ...
    def parse(self, response: ModelResponse) -> ModelResponse:
        ...
        # Find the JSON block - use unified approach for all cases
        search_pos = 0
        last_error = None

        while search_pos < len(text):
            # Try to find the specified tag_start first
            start_idx = text.find(self.tag_start, search_pos)

            if start_idx == -1:
                # If tag_start not found, try fallback patterns
                start_idx = text.find("```json", search_pos)
                if start_idx == -1:
                    # Try plain ``` if ```json not found
                    start_idx = text.find("```", search_pos)
                    if start_idx == -1:
                        break
            ...
            # Check if immediately followed by another ```json or ``` (empty block)
            if (
                text[content_start : content_start + 7] == "```json"
                or text[content_start : content_start + 3] == "```"
            ):
                # Empty block, continue searching
            ...
            if end_idx == -1:
                last_error = f"End tag '```' not found in response"
                break
            ...
            if json_content:
                try:
                    ...
                    # If parse fails, try the next block
