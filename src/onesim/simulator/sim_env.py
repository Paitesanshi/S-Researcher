...
class BasicSimEnv:
    ...
    def __init__(
        ...
        if isinstance(config, dict):
            ...
            self.config = SimulationConfig(
                ...
                export_training_data=config.get('export_training_data', True),
                ...
                collection_interval=config.get('collection_interval', 120),
            )
        elif config is None:
            self.config = SimulationConfig()
        ...
        # Track outstanding websocket broadcast tasks (created via asyncio.create_task)
        # so we can cancel them on stop/reset and avoid "late" events leaking into
        # the next run or next websocket connection.
        ...
        # Run initialization steps synchronously and block until done
        self.initialize()

    def initialize(self):
        """Perform synchronous initialization steps. Must block until initial data is loaded."""
        self.data["simulation_start_time"] = time.time()
        # Ensure load_initial_data completes before proceeding (now synchronous)
        self.load_initial_data()
        # Snapshot initial state for clean restart via reset().
        # NOTE: This snapshot includes env_data.json merged into initial data.
        self._initial_data_snapshot = copy.deepcopy(self.data)
    ...
    async def _save_step_data(self, step_num: int):
        ...
        try:
            ...
            if self.config.export_training_data:
                ...
                if not dataset_dir:
                    logger.warning(
                        "Training data export skipped for this step: datasets directory "
                        "is not available (output_dir is None)."
                    )
                else:
                    ...
                    export_path = os.path.join(dataset_dir, f"decisions.json")
                    ...
                    try:
                        ...
                        logger.error(
                            f"Failed to create dataset directory {dataset_dir}: {e}"
                        )
                    ...
                    try:
                        ...
                        if self.trail_id:
                            try:
                                ...
                                data = await decision_mgr.export_training_data(
                                    trail_id=self.trail_id
                                )

                                # Use aiofiles for writing
                                async with aiofiles.open(export_path, "w") as f:
                                    # Assuming data is already a JSON string
                                    await f.write(data)
                                logger.info(
                                    f"Exported training data to {export_path} from database"
                                )
                            except Exception as db_error:
                                logger.warning(
                                    f"Failed to export data from database: {db_error}. Falling back to direct export."
                                )
                                # Make the fallback async
                                await self._export_data_from_pending_decisions(
                                    export_path
                                )
                        else:
            ...
            if monitor_dir:
                try:
                    ...
                    if monitor_manager:
                        # 先更新所有指标以确保数据是最新的
                        logger.info(
                            f"Updating all metrics before exporting for step {self.current_step}"
                        )
                        await monitor_manager.update_all_metrics(self)
    ...
    async def _execute_interventions(self, step_num: int):
        """Execute runtime interventions at the end of each step"""
        if not self.intervention_engine:
            return

        try:
            # Get scheduled interventions for this step
            scheduled_interventions = (
                self.intervention_engine.get_scheduled_interventions(step_num)
            )
            for intervention_item in scheduled_interventions:
                await self.intervention_engine.execute_intervention(
                    intervention_item['spec'], self
                )

            # Check conditional interventions
            simulation_context = {
                'current_step': step_num,
                'environment': self,
                'agents': self.agents,
            }
            conditional_interventions = (
                self.intervention_engine.get_conditional_interventions(
                    simulation_context, step_num
                )
            )
            for intervention_item in conditional_interventions:
                await self.intervention_engine.execute_intervention(
                    intervention_item['spec'], self
                )

        except Exception as e:
            logger.error(f"Error executing interventions at step {step_num}: {e}")
    ...
    async def queue_event(self, event_data: Dict[str, Any]):
        ...
        # Hard guard: once simulation is terminated/completed, do not queue/buffer any more
        # business events. Otherwise late agent tasks can leak "old" events into the next run
        # (e.g., when frontend reconnects and buffered events are flushed).
        #
        # Allow EndEvent through (optional) so frontend can still see the termination marker.
        try:
            if self.is_terminated() and event_type not in ["EndEvent"]:
                return
        except Exception:
            # If state check fails, be conservative and keep previous behavior.
            pass

        # Step-based stale event dropping:
        # If we've already entered the next step, any event created BEFORE the current step's
        # start time but only arriving now is considered "late from previous round" and should
        # be ignored to avoid mixing rounds on the frontend.
        #
        # This relies on Event.timestamp (seconds) and the fact that ROUND mode sets
        # self.data[f"step_{current_step}_time_start"] at the beginning of each step.
        try:
            if (
                event_type not in ["EndEvent"]
                and hasattr(self, "current_step")
                and isinstance(getattr(self, "current_step", None), int)
                and self.current_step > 1
            ):
                current_step_start = self.data.get(
                    f"step_{self.current_step}_time_start"
                )
                evt_ts = event_data.get("timestamp")
                if (
                    isinstance(current_step_start, (int, float))
                    and isinstance(evt_ts, (int, float))
                    and evt_ts < current_step_start
                ):
                    return
        except Exception:
            # Best-effort filter; never break event pipeline on errors.
            pass
        ...
        if event_type in [
            ...
            return
        if not event_data.get('from_action') or not event_data.get('to_action'):
            return

        # Get environment name from path
        env_name = os.path.basename(self.env_path) if self.env_path else self.name
        
        # Step should already be set through inheritance from parent event
        # Only set as fallback if missing (this shouldn't happen in normal flow)
        if 'step' not in event_data or event_data['step'] is None:
            logger.warning(f"Event {event_type} missing step information, using current step {self.current_step}")
        ...
        if connection_manager.has_active_connections(env_name):
            ...
            try:
                ...
                self._broadcast_tasks.add(t)
                t.add_done_callback(lambda _t: self._broadcast_tasks.discard(_t))
            except Exception:
                # Fallback: best-effort direct await (should be rare)
                await connection_manager.broadcast_event(env_name, event_data)
        else:
    ...
    def load_initial_data(self):
        """Load initial environment data synchronously."""
        ...
        if env_file and os.path.exists(env_file):
            try:
                with open(env_file, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                # Safe to update directly during initialization
    ...
    async def start(self, **kwargs: Any) -> None:
        ...
        # Execute runtime interventions if enabled
        await self._execute_interventions(self.current_step)
        ...
        if self.mode == SimulationMode.ROUND:
            ...
            for agent_type in self.start_targets.keys():
                for target_id in self.start_targets[agent_type]:
                    start_event = await self._create_start_event(target_id)
                    # Critical: Set the correct step at the source (StartEvent)
                    start_event.step = self.current_step
    ...
    async def update_data(self, key: str, data: Any) -> bool:
        ...
        Returns:
            bool: True when the update is applied successfully.
        """
        # Use asyncio.Lock
        async with self._lock:
    ...
    async def stop_simulation(self):
        ...
        # Defensive: once terminated, drop any buffered events so they cannot be flushed
        # to the frontend on the next websocket connection.
        if hasattr(self, "_buffered_events"):
            try:
                self._buffered_events.clear()
            except Exception:
                self._buffered_events = []

        # Cancel any in-flight websocket broadcast tasks to avoid late sends after stop
        if hasattr(self, "_broadcast_tasks") and self._broadcast_tasks:
            tasks = list(self._broadcast_tasks)
            for t in tasks:
                try:
                    if not t.done():
                        t.cancel()
        ...
        if (
            self.config.export_event_data
            and hasattr(self, '_pending_events')
            ...
            try:
                ...
                filename = (
                    f"{self.trail_id or f'simulation_events_{int(time.time())}'}.json"
                )
                export_path = os.path.join(dataset_dir, filename)
                await self._export_data_from_pending_events(export_path)
                logger.info(
                    f"Exported {len(self._pending_events)} events to {export_path}"
                )
            except Exception as e:
                logger.error(f"Error exporting event data: {e}")
        else:
            logger.info(
                f"Event export skipped: export_event_data={self.config.export_event_data}, "
                f"has_pending_events={hasattr(self, '_pending_events')}, "
                f"pending_count={len(self._pending_events) if hasattr(self, '_pending_events') else 0}"
            )
        ...
        if self.config.export_training_data:
            ...
            if not dataset_dir:
                logger.warning(
                    "Training data export skipped: datasets directory is not available "
                    "(output_dir is None)."
                )
            else:
                ...
                export_path = os.path.join(dataset_dir, "decisions.json")
                    ...
                    if self.trail_id:
                        try:
                            ...
                            with open(export_path, "w") as f:
                                f.write(data)  # Assuming data is already JSON string
                            logger.info(
                                f"Exported training data to {export_path} from database"
                            )
                        except Exception as db_error:
                        ...
                        # Direct export from pending decisions when not using database
                        await self._export_data_from_pending_decisions(export_path)
                except Exception as e:
                    logger.error(f"Error exporting training data: {e}")
    ...
    async def reset(self):
        """
        Reset simulation to initial state for clean restart.

        This method performs complete cleanup of all accumulated state:
        - Clears all pending events and decisions
        - Resets counters and timers
        - Cleans up caches and buffers
        - Restores initial state

        Call this before restarting a stopped simulation to ensure clean state.
        """
        logger.info(f"Resetting simulation environment {self.name} to initial state")

        # Best-effort: stop periodic metrics task if it's still running
        if (
            hasattr(self, '_metrics_collection_task')
            and self._metrics_collection_task is not None
        ):
            if not self._metrics_collection_task.done():
                self._metrics_collection_task.cancel()
                await asyncio.gather(
                    self._metrics_collection_task, return_exceptions=True
                )
            self._metrics_collection_task = None

        # Best-effort: stop scheduler (timed mode) to avoid leftover scheduled triggers
        ...
        # Cancel any in-flight websocket broadcast tasks
        if hasattr(self, "_broadcast_tasks") and self._broadcast_tasks:
            tasks = list(self._broadcast_tasks)
            for t in tasks:
                try:
        ...
        # Reset ended_agents tracking
        self.ended_agents = {id: 0 for ids in self.end_targets.values() for id in ids}

        # Clear buffered events if they exist
        if hasattr(self, '_buffered_events'):
            self._buffered_events.clear()
        ...
        # Clear pause and termination signals
        self._pause_signal.clear()
        self._termination_signal.clear()
        if hasattr(self, 'stopped_event'):
            self.stopped_event.clear()

        # Restore environment data to the initial snapshot (if available)
        # This prevents mutated env data from leaking into the next run when reusing the same instance.
        if hasattr(self, "_initial_data_snapshot"):
            self.data = copy.deepcopy(self._initial_data_snapshot)
        else:
            # Fallback: at least clear step/timing markers
            if 'step_data' in self.data:
                self.data['step_data'].clear()
            for key in list(self.data.keys()):
                if key.startswith('step_') and (
                    '_time_start' in key or '_time_end' in key
                ):
                    del self.data[key]

        # Always reset simulation timing fields
        self.data["simulation_start_time"] = time.time()
        self.data.pop("simulation_complete", None)
        self.data.pop("simulation_end_time", None)
        self.data.pop("total_simulation_time", None)
        ...
        # Clear data futures and sync events
        if hasattr(self, '_data_futures'):
            self._data_futures.clear()
        if hasattr(self, '_data_update_futures'):
            self._data_update_futures.clear()
        if hasattr(self, '_data_sync_events'):
            self._data_sync_events.clear()
        if hasattr(self, '_data_update_sync_events'):
            self._data_update_sync_events.clear()

        logger.info(f"Simulation environment {self.name} reset complete")
