...
class WorkGraph(metaclass=SingletonMeta):
    ...
    def get_nodes_by_event_name(self, event_name: str) -> Optional[Tuple[str, str]]:
        """
        Get the source and target node IDs for an event by its name.
        
        :param event_name: The name of the event
        :return: A tuple of (from_node_id, to_node_id) or None if not found
        """
        with self._lock:
            for from_node, to_node, edge_data in self.graph.edges(data=True):
                if edge_data.get('event_name') == event_name:
    ...
    def get_action_names_by_event_name(self, event_name: str) -> Optional[Tuple[str, str]]:
        """
        Get the source and target action names for an event by its name.
        
        :param event_name: The name of the event
        :return: A tuple of (from_action_name, to_action_name) or None if not found
        """
        nodes = self.get_nodes_by_event_name(event_name)
        if nodes is None:
            return None
        
        from_node_id, to_node_id = nodes
        with self._lock:
            # Extract action name from node_id format: {agent_type}.{action_name}
            from_action = self.graph.nodes[from_node_id]['name'] if self.graph.has_node(from_node_id) else None
            to_action = self.graph.nodes[to_node_id]['name'] if self.graph.has_node(to_node_id) else None
